...
class Evaluation_Metric():
	def __init__(self, eval_metric="bleu"):
		self.eval_metric = eval_metric
		self.weights = [0.25, 0.25, 0.25, 0.25]

	def bleu(self,candidate, references, weights):
	    ...
	    """
	    p_ns = (
	        self._modified_precision(candidate, references, i)
	...
	def evaluate(self, actual,generated):
		if self.eval_metric == 'bleu':
			return np.mean([self.bleu(actual[key],[generated[key]],self.weights) for key in actual.keys()])
